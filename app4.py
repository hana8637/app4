...
# ==============================================================================
# ==============================================================================
# 공통 설정 (한글 폰트 웹 완벽 대응 - 수정됨)
# ==============================================================================
@st.cache_resource
def set_korean_font():
    """웹(Linux) 및 로컬 환경 모두 한글이 깨지지 않도록 폰트를 설정합니다."""
    import platform
    import os
    import matplotlib.pyplot as plt
    import matplotlib.font_manager as fm
    ...
    if os_name == "Windows":
        ...
        try:
            ...
            # 폰트 매니저에 등록된 나눔고딕의 정확한 영문 이름을 찾아 설정
            font_names = [f.name for f in fm.fontManager.ttflist]
            if 'NanumGothic' in font_names:
                plt.rc('font', family='NanumGothic')
            else:
                # 폰트 이름이 조금 다를 경우를 대비한 백업
                plt.rc('font', family='Nanum Gothic')
        except Exception as e:
            st.error(f"폰트 다운로드/설정 중 오류 발생: {e}")
...
def generate_custom_truss(type_choice, span_cm, divs, h_outer_cm, h_center_cm, h_tie_cm, m_od, v_od, r_od, d_od, offset_mm):
    """트러스 도면 생성 코어 함수"""
    name_map = {
    ...
    t_name = name_map.get(type_choice, "밑더블_삼각")
    ...
    def draw_dim_text(ax, x, y, text, angle=0, color='black', fontsize=11.5):
        ...
        ax.text(x, y, text, color=color, fontsize=fontsize, fontweight='bold', ha='center', va='center', rotation=angle,
                bbox=dict(facecolor='white', alpha=0.85, edgecolor='black', pad=1.5)) # 테두리 추가로 진하게

    fig, ax = plt.subplots(figsize=(40, 18), dpi=100)
    mid_idx = divs if is_half else divs // 2
    raw_data = []

    # 수정된 다대 좌표 계산 (버그 픽스 유지)
    v_centers_x = [v_od/2 if i==0 else (S - v_od/2 if i==divs else i*(S/divs)) for i in range(divs + 1)]
    ...
    if is_double_bot:
        ...
        ax.add_patch(patches.Rectangle((0, H_mid_bot), total_mid_pipe_len, m_od, facecolor='#5B2C6F', edgecolor='black', zorder=6)) # 색상 진하게
        draw_dim_text(ax, S/2, H_mid_bot + m_od/2, f"밑더블 수평재(전체) L:{total_mid_pipe_len:.1f}", angle=0, color='#4A235A', fontsize=14)
        raw_data.append({
    ...
    for i in range(divs + 1):
        ...
        if is_double_bot:
            ...
            if u_len > 0:
                ...
                ax.add_patch(patches.Polygon(pts_u, facecolor='#1A5276', edgecolor='black', zorder=5)) # 색상 진하게
                ...
                ax.plot([x, x], [y_top_c + m_od/2, my_top - 180], color='darkblue', linestyle='-', lw=2, zorder=1)
                draw_dim_text(ax, x, my_top, f"{g_name}\nL:{u_cut_max:.1f}", angle=90, color='darkblue', fontsize=11)
            ...
            if l_len > 0:
                ...
                ax.add_patch(patches.Polygon(pts_l, facecolor='#1C2833', edgecolor='black', zorder=5)) # 색상 진하게
                ...
                ax.plot([x, x], [y_bot_c - m_od/2, my_bot + 180], color='navy', linestyle='-', lw=2, zorder=1)
                draw_dim_text(ax, x, my_bot, f"{g_name}\nL:{l_cut_max:.1f}", angle=90, color='navy', fontsize=11)
        else:
            pts_v = poly_bot + poly_top
            ax.add_patch(patches.Polygon(pts_v, facecolor='#17202A', edgecolor='black', zorder=5)) # 색상 진하게
            
            text_color = 'darkred' if is_ridge else 'darkblue'
            stagger_offset = 600 if i % 2 == 0 else 900
            my = y_top_c + stagger_offset
            ax.plot([x, x], [y_top_c + m_od/2, my - 180], color=text_color, linestyle='-', lw=2, zorder=1)
    ...
    for i in range(divs):
        ...
        ax.add_patch(patches.Polygon(np.array([pb1, pb2, pb3, pb4]), facecolor='#566573', alpha=0.8, zorder=2)) # 색상 진하게
        
        pt1, pt2 = (cx, get_chord_y_top(cx)), (cnx, get_chord_y_top(cnx))
        pt3, pt4 = (cnx, get_y_top(cnx)), (cx, get_y_top(cx))
        ax.add_patch(patches.Polygon(np.array([pt1, pt2, pt3, pt4]), facecolor='#566573', zorder=7))
        ...
        if is_diag:
            ...
            def draw_diag(left_x, right_x, is_forward):
                ...
                poly = plt.Polygon(pts, facecolor='#D4AC0D', edgecolor='black', linewidth=1.5, zorder=3) # 색상 진하게
                ax.add_patch(poly)
                ...
                draw_dim_text(ax, mx, my, f"L:{diag_l:.1f} ({d_top_angle}°/{d_bot_angle}°)", angle=diag_ang, color='darkred', fontsize=12)
            ...
            if is_double_bot:
                def draw_custom_diag(left_x, right_x, gubun_name, is_forward):
                    ...
                    poly = plt.Polygon(pts, facecolor='#D4AC0D', edgecolor='black', linewidth=1.5, zorder=3)
                    ax.add_patch(poly)
                    ...
                    mx, my = (px_bot + px_top)/2, (py_bot + py_top)/2
                    draw_dim_text(ax, mx, my, f"L:{diag_l:.1f} ({d_top_angle}°/{d_bot_angle}°)", angle=diag_ang, color='darkred', fontsize=11)
    ...
    if has_tie and H_tie > 0:
        ...
        if tie_length_inner > 0:
            ax.add_patch(patches.Rectangle((x_left_in, H_tie - m_od/2), tie_length_inner, m_od, facecolor='#5B2C6F', edgecolor='black', zorder=6))
            ax.annotate(f"수평재 내경 L:{tie_length_inner:.1f}", xy=((x_left_in+x_right_in)/2, H_tie), xytext=((x_left_in+x_right_in)/2, H_tie - 120),
                        arrowprops=dict(arrowstyle='<->', color='indigo', lw=3), ha='center', fontsize=16, fontweight='bold', color='indigo')
        ...
        for i, x in valid_inner_xs:
            ...
            ax.add_patch(patches.Polygon(pts_inner, facecolor='#1E8449', edgecolor='black', zorder=4))
            ...
            ax.plot([x, x], [H_tie - m_od/2, my_bot + 180], color='darkgreen', linestyle='-', lw=2, zorder=1)
        ...
        if type_choice == "5":
            ...
            ax.plot([dim_h_x - 100, dim_h_x + 100], [0, 0], color='black', lw=2, zorder=1)
            ax.annotate("", xy=(dim_h_x, tie_bot_y), xytext=(dim_h_x, 0),
                        arrowprops=dict(arrowstyle='<->', color='#A04000', lw=3))
            draw_dim_text(ax, dim_h_x, tie_bot_y / 2, f"수평보 하단 높이: {tie_bot_y:.1f}", angle=90, color='#A04000', fontsize=14)
        ...
        for i in range(len(inner_intervals)-1):
            ...
            if wx_end > wx_start + 10: 
                ...
                if is_forward_tie:
                    dy_est = get_y_bot(wx_end) - y_bot_limit
                    diag_l = math.hypot(dx_est, dy_est)
                else:
                    dy_est = get_y_bot(wx_start) - y_bot_limit
                    diag_l = math.hypot(dx_est, dy_est)
                ...
                mx, my = (px_bot + px_top)/2, (py_bot + py_top)/2
                draw_dim_text(ax, mx, my, f"L:{diag_l:.1f} ({d_top_angle}°/{d_bot_angle}°)", angle=diag_ang, color='#A04000', fontsize=12)
    ...
    ax.plot([0, S], [dim_y, dim_y], color='black', lw=3, zorder=10)
    ax.plot([0, 0], [dim_y - 25, dim_y + 25], color='black', lw=3, zorder=10)
    ax.plot([S, S], [dim_y - 25, dim_y + 25], color='black', lw=3, zorder=10)
    ...
    for i in range(divs):
        ...
        if i > 0: 
            ax.plot([tx1, tx1], [dim_y - 20, dim_y + 20], color='black', lw=2, zorder=10)
            ax.plot([tx1, tx1], [0, dim_y], color='black', linestyle=':', lw=1.5, zorder=1)
        
        f_size = 14 if interval_len > 300 else 12
        ax.text(cx, dim_y + 60, f"{interval_len:.1f}", ha='center', va='center', fontsize=f_size, color='navy', fontweight='bold')

    for tx in ticks_x:
        ax.text(tx, dim_y - 45, f"{tx:.1f}", ha='center', va='top', fontsize=12, color='#8B0000', fontweight='bold')

    ax.plot([S, S], [0, dim_y], color='black', linestyle=':', lw=1.5, zorder=1)
    ax.text(S/2, dim_y - 120, f"전체 스판 : {S:.1f} mm", ha='center', va='center', fontsize=20, fontweight='bold', color='black')
    ...
    plt.title(f"트러스 도면 ({t_name})\n{info_text}", fontsize=30, fontweight='bold', pad=30)
    plt.tight_layout()
    
    return fig, raw_data, t_name
...
def draw_pipe(ax, x1, y1, x2, y2, t, zorder=1, facecolor='white'):
    ...
    poly = plt.Polygon(
        ...
        facecolor=facecolor, edgecolor='black', linewidth=1.5, zorder=zorder # 라인 굵기 증가
    )
    ax.add_patch(poly)
...
def draw_diag_poly(ax, x_start_tip, x_end_tip, y_bot, y_top, w_half, is_forward, zorder=1, facecolor='yellow'):
    ...
    poly = plt.Polygon(pts, facecolor=facecolor, edgecolor='black', linewidth=1.5, zorder=zorder) # 라인 굵기 증가
    ax.add_patch(poly)

def run_ladder_system(L_cm, W_cm, H_truss_cm, total_sets_sub, total_sets_main, offset_mm, H_ridge_cm, ridge_deduct_mm, total_sets_ridge, wall_snagi_mm, p_sub_main, p_sub_sub, p_main_main, p_main_snagi, p_main_v, p_main_diag, p_ridge_main, p_ridge_v, p_ridge_diag):
    ...
    def calc_diag(spacing, v_len, left_r, right_r, t_diag):
        ...
        W_half = min(W_half, dx_center * 0.45) 
        
        return actual_diag, cut_angle, angle_deg, W_half

    # --- 보강 사다리 계산 ---
    actual_sub_v_len = W_cm - (2 * t_sub_main_cm)
    diag_sub_norm, cut_sub_norm, angle_sub_norm, w_half_sub_norm = calc_diag(sub_gap, actual_sub_v_len, t_sub_sub_cm/2, t_sub_sub_cm/2, t_sub_sub_cm)
    S_sub_edge = sub_gap - t_sub_sub_cm/2
    diag_sub_edge, cut_sub_edge, angle_sub_edge, w_half_sub_edge = calc_diag(S_sub_edge, actual_sub_v_len, t_sub_sub_cm/2, t_sub_sub_cm/2, t_sub_sub_cm)

    # --- 메인 사다리 계산 ---
    actual_main_v_len = H_truss_cm - (2 * t_main_main_cm)
    diag_main_norm, cut_main_norm, angle_main_norm, w_half_main_norm = calc_diag(sub_gap, actual_main_v_len, t_main_v_cm/2, t_main_v_cm/2, t_main_diag_cm)
    diag_main_snagi, cut_main_snagi, angle_main_snagi, w_half_main_snagi = calc_diag(sub_gap, actual_main_v_len, t_main_snagi_cm/2, t_main_v_cm/2, t_main_diag_cm)
    S_main_edge = sub_gap - t_main_snagi_cm/2
    diag_main_edge, cut_main_edge, angle_main_edge, w_half_main_edge = calc_diag(S_main_edge, actual_main_v_len, t_main_snagi_cm/2, t_main_v_cm/2, t_main_diag_cm)
    ...
    S_norm = sub_gap
    S_mid_deep = sub_gap - (ridge_deduct_cm / 2) - (t_ridge_v_cm / 2)
    S_end_deep = sub_gap - ridge_deduct_cm - (t_ridge_v_cm / 2)

    diag_norm, cut_norm, _, w_half_r_norm = calc_diag(S_norm, actual_ridge_v_len, t_ridge_v_cm/2, t_ridge_v_cm/2, t_ridge_diag_cm)
    diag_mid_c, cut_mid_c, _, w_half_r_mid = calc_diag(S_mid_deep, actual_ridge_v_len, t_ridge_v_cm/2, t_ridge_v_cm/2, t_ridge_diag_cm)
    diag_end_c, cut_end_c, _, w_half_r_end = calc_diag(S_end_deep, actual_ridge_v_len, t_ridge_v_cm/2, t_ridge_v_cm/2, t_ridge_diag_cm)
    ...
    H_snagi_len = round((H_truss_cm - t_main_main_cm) - (-30), 1)
    ...
    if n_sec_s > 2:
        qty_sub_norm = (n_sec_s - 2) * total_sets_sub
        data.append([f"[보강사다리] 사재-일반({int(round(cut_sub_norm, 0))}°)", f"{p_sub_sub}mm", qty_sub_norm, round(diag_sub_norm, 1), round(diag_sub_norm*qty_sub_norm, 1), get_6m_count(diag_sub_norm*qty_sub_norm)])
    
    qty_sub_edge = 2 * total_sets_sub
    data.append([f"[보강사다리] 사재-양끝({int(round(cut_sub_edge, 0))}°)", f"{p_sub_sub}mm", qty_sub_edge, round(diag_sub_edge, 1), round(diag_sub_edge*qty_sub_edge, 1), get_6m_count(diag_sub_edge*qty_sub_edge)])
    
    data.append(["------------------", "----------", "---", "---", "---", "---"])
    
    data.extend([
        ["[메인사다리] 상하현재", f"{p_main_main}mm", 2 * total_sets_main, L_cm, round((L_cm*2)*total_sets_main, 1), get_6m_count((L_cm*2)*total_sets_main)],
        ["[메인사다리] 스나기", f"{p_main_snagi}mm", (n_sec_m+1) * total_sets_main, H_snagi_len, round(((n_sec_m+1)*H_snagi_len)*total_sets_main, 1), get_6m_count(((n_sec_m+1)*H_snagi_len)*total_sets_main)],
    ])
    
    qty_main_v = n_sec_m * (sub_div - 1) * total_sets_main
    if qty_main_v > 0:
        data.append(["[메인사다리] 수직다대", f"{p_main_v}mm", qty_main_v, round(actual_main_v_len, 1), round(qty_main_v*actual_main_v_len, 1), get_6m_count(qty_main_v*actual_main_v_len)])

    qty_main_diag_edge = 2 * total_sets_main
    data.append([f"[메인사다리] 살대-스나기접합(양끝)({int(round(cut_main_edge, 0))}°)", f"{p_main_diag}mm", qty_main_diag_edge, round(diag_main_edge,1), round(diag_main_edge*qty_main_diag_edge, 1), get_6m_count(diag_main_edge*qty_main_diag_edge)])

    qty_main_diag_snagi = 2 * (n_sec_m - 1) * total_sets_main
    if qty_main_diag_snagi > 0:
        data.append([f"[메인사다리] 살대-스나기접합(일반)({int(round(cut_main_snagi, 0))}°)", f"{p_main_diag}mm", qty_main_diag_snagi, round(diag_main_snagi,1), round(diag_main_snagi*qty_main_diag_snagi, 1), get_6m_count(diag_main_snagi*qty_main_diag_snagi)])

    qty_main_diag_norm = (sub_div - 2) * n_sec_m * total_sets_main
    if qty_main_diag_norm > 0:
        data.append([f"[메인사다리] 살대-일반({int(round(cut_main_norm, 0))}°)", f"{p_main_diag}mm", qty_main_diag_norm, round(diag_main_norm,1), round(diag_main_norm*qty_main_diag_norm, 1), get_6m_count(diag_main_norm*qty_main_diag_norm)])
    ...
    ax1.annotate('', xy=(0, W_cm*1.3), xytext=(L_cm, W_cm*1.3), arrowprops=dict(arrowstyle='<->', color='red', lw=3))
    ax1.text(L_cm/2, W_cm*1.35, f"총기장: {L_cm}cm", ha='center', color='darkred', weight='bold', fontsize=28) 

    # 색상 진하게: '#C0C0C0' -> '#696969'
    draw_pipe(ax1, 0, y_top_center_s, L_cm, y_top_center_s, t_sub_main_cm, zorder=3, facecolor='#696969')
    draw_pipe(ax1, 0, y_bot_center_s, L_cm, y_bot_center_s, t_sub_main_cm, zorder=3, facecolor='#696969')
    ...
    for i in range(n_sec_s):
        ...
        d_len, c_ang, a_rad, w_h = calc_diag(span_dist, actual_sub_v_len, t_sub_sub_cm/2, t_sub_sub_cm/2, t_sub_sub_cm)
        
        draw_pipe(ax1, x, y_inner_bot_s, x, y_inner_top_s, t_sub_sub_cm, zorder=2, facecolor='navy')
        if i == n_sec_s - 1:
            draw_pipe(ax1, nx_val, y_inner_bot_s, nx_val, y_inner_top_s, t_sub_sub_cm, zorder=2, facecolor='navy')
        ...
        ax1.text((grid_x + grid_nx)/2, -W_cm*0.2, span_label, ha='center', fontsize=18, color='black', weight='bold') 
        
        ax1.plot([grid_x, grid_x], [-W_cm*0.1, y_inner_bot_s], color='black', linestyle=':', lw=2)
        if i == n_sec_s - 1:
            ax1.plot([grid_nx, grid_nx], [-W_cm*0.1, y_inner_bot_s], color='black', linestyle=':', lw=2)
        
        start_tip_s = x + (t_sub_sub_cm / 2) + dx_s
        end_tip_s = nx_val - (t_sub_sub_cm / 2) - dx_s
        
        if i % 2 == 0: 
            draw_diag_poly(ax1, start_tip_s, end_tip_s, y_inner_bot_s, y_inner_top_s, w_h, True, zorder=1, facecolor='#D4AC0D')
            ax1.text((start_tip_s + end_tip_s)/2, W_cm/2, f"{round(d_len,1)}\n({int(round(c_ang, 0))}°)", ha='center', va='center', fontsize=16, color='black', rotation=a_rad, weight='bold') 
        else:          
            draw_diag_poly(ax1, start_tip_s, end_tip_s, y_inner_bot_s, y_inner_top_s, w_h, False, zorder=1, facecolor='#D4AC0D')
            ax1.text((start_tip_s + end_tip_s)/2, W_cm/2, f"{round(d_len,1)}\n({int(round(c_ang, 0))}°)", ha='center', va='center', fontsize=16, color='black', rotation=-a_rad, weight='bold') 

    ax1.text(-gap_s*0.1, W_cm/2, f"다대 실절단\n{round(actual_sub_v_len,1)}cm", va='center', ha='right', color='black', fontsize=24, weight='bold') 
    ax1.set_title(f"1. 보강사다리 상세 (폭: {W_cm}cm, 양끝 다대 외경-싱 기준 적용)", fontsize=36, fontweight='bold', pad=30) 
    ...
    ax2.annotate('', xy=(0, y_total_line), xytext=(L_cm, y_total_line), arrowprops=dict(arrowstyle='<->', color='red', lw=3))
    ax2.text(L_cm/2, y_total_line + 5, f"총기장: {L_cm}cm", ha='center', color='darkred', weight='bold', fontsize=28) 

    draw_pipe(ax2, 0, y_top_center_m, L_cm, y_top_center_m, t_main_main_cm, zorder=3, facecolor='#696969')
    draw_pipe(ax2, 0, y_bot_center_m, L_cm, y_bot_center_m, t_main_main_cm, zorder=3, facecolor='#696969')
    ...
    for i in range(n_sec_m + 1):
        ...
        draw_pipe(ax2, snagi_cx, -30, snagi_cx, y_inner_top_m, t_main_snagi_cm, zorder=5, facecolor='indigo')
        if i == 0:
            ax2.annotate('', xy=(snagi_cx - t_main_snagi_cm, 0), xytext=(snagi_cx - t_main_snagi_cm, -30), arrowprops=dict(arrowstyle='<->', color='indigo', lw=2))
            ax2.text(snagi_cx - t_main_snagi_cm - 2, -15, "30cm", ha='right', va='center', color='indigo', fontsize=20, weight='bold') 

    for i in range(n_sec_m):
        cx = snagi_cx_list[i]
        nxt_cx = snagi_cx_list[i+1]
        
        base_x = i * gap_m
        v_pipes = [base_x + j * sub_gap for j in range(1, sub_div)]
        nodes = [cx] + v_pipes + [nxt_cx]
        
        ax2.text((base_x + base_x + gap_m)/2, H_truss_cm + 35, f"구분간격(벽사다리 스팬): {round(gap_m,1)}cm", ha='center', color='black', weight='bold', fontsize=24) 
        ...
        for j in range(sub_div):
            ...
            if j < sub_div - 1:
                draw_pipe(ax2, v_pipes[j], y_inner_bot_m, v_pipes[j], y_inner_top_m, t_main_v_cm, zorder=4, facecolor='navy')
            
            grid_x = base_x + j * sub_gap
            grid_nx = base_x + (j + 1) * sub_gap
            
            global_idx = i * sub_div + j
            if global_idx == 0: span_label = f"[스나기외경~싱]\n{round(sub_gap,1)}cm"
            elif global_idx == n_sec_m * sub_div - 1: span_label = f"[싱~스나기외경]\n{round(sub_gap,1)}cm"
            else: span_label = f"[싱~싱]\n{round(sub_gap,1)}cm"
                
            ax2.text((grid_x + grid_nx)/2, -25, span_label, ha='center', fontsize=18, color='black', weight='bold') 
            
            ax2.plot([grid_x, grid_x], [-15, y_inner_bot_m], color='black', linestyle=':', lw=2)
            if global_idx == n_sec_m * sub_div - 1:
                ax2.plot([grid_nx, grid_nx], [-15, y_inner_bot_m], color='black', linestyle=':', lw=2)
            
            l_r = (t_main_snagi_cm / 2) if j == 0 else (t_main_v_cm / 2)
            r_r = (t_main_snagi_cm / 2) if j == sub_div - 1 else (t_main_v_cm / 2)
            
            d_len, c_ang, a_rad, w_h = calc_diag(span_dist, actual_main_v_len, l_r, r_r, t_main_diag_cm)
            
            start_tip_m = px + l_r + dx_m
            end_tip_m = curx - r_r - dx_m
            
            if j % 2 != 0:
                draw_diag_poly(ax2, start_tip_m, end_tip_m, y_inner_bot_m, y_inner_top_m, w_h, True, zorder=3, facecolor='#D4AC0D')
                ax2.text((start_tip_m + end_tip_m)/2, H_truss_cm/2, f"{round(d_len,1)}\n({int(round(c_ang, 0))}°)", ha='center', va='center', fontsize=16, color='black', rotation=a_rad, weight='bold') 
            else:
                draw_diag_poly(ax2, start_tip_m, end_tip_m, y_inner_bot_m, y_inner_top_m, w_h, False, zorder=3, facecolor='#D4AC0D')
                ax2.text((start_tip_m + end_tip_m)/2, H_truss_cm/2, f"{round(d_len,1)}\n({int(round(c_ang, 0))}°)", ha='center', va='center', fontsize=16, color='black', rotation=-a_rad, weight='bold') 
    ...
    for i in range(n_sec_m):
        ...
        ax2.text(span_center, -75, f"▼ 벽사다리({w_label}) 가로 절단(스나기 {wall_snagi_mm}mm): {round(wall_len, 1)}cm", 
                 ha='center', color='darkgreen', weight='bold', fontsize=22) 

    ax2.text(-gap_m*0.1, H_truss_cm/2, f"다대 실절단\n{round(actual_main_v_len,1)}cm", va='center', ha='right', color='black', fontsize=24, weight='bold') 
    ax2.set_title(f"2. 메인사다리 상세 (양끝 스나기 외경-싱 기준 적용)", fontsize=36, fontweight='bold', pad=30) 
    ax2.set_xlim(-L_cm*0.08, L_cm*1.08)
    ax2.set_ylim(-100, H_truss_cm + 100)
    ...
    ax3.annotate('', xy=(0, H_ridge_cm + 65), xytext=(L_cm, H_ridge_cm + 65), arrowprops=dict(arrowstyle='<->', color='red', lw=3))
    ax3.text(L_cm/2, H_ridge_cm + 70, f"총기장: {L_cm}cm", ha='center', color='darkred', weight='bold', fontsize=28) 

    for i in range(n_sec_m + 1):
        cx = i * gap_m
        if i == 0: snagi_cx = ridge_deduct_cm / 2  
        elif i == n_sec_m: snagi_cx = L_cm - (ridge_deduct_cm / 2) 
        else: snagi_cx = cx  
        draw_pipe(ax3, snagi_cx, -30, snagi_cx, H_ridge_cm, ridge_deduct_cm, zorder=5, facecolor='indigo')
        ...
        if i < n_sec_m:
            ...
            if n_sec_m == 1:
                ...
                v_centers = [x_min + t_ridge_v_cm/2, cx_start + 1*sub_gap, cx_start + 2*sub_gap, cx_start + 3*sub_gap, x_max - t_ridge_v_cm/2]
                label = "[단일세트]"
            elif i == 0:
                x_min = ridge_deduct_cm
            ...
            draw_pipe(ax3, x_min, y_top_center_r, x_max, y_top_center_r, t_ridge_main_cm, zorder=3, facecolor='#696969')
            draw_pipe(ax3, x_min, y_bot_center_r, x_max, y_bot_center_r, t_ridge_main_cm, zorder=3, facecolor='#696969')

            for vx in v_centers:
                draw_pipe(ax3, vx, y_inner_bot_r, vx, y_inner_top_r, t_ridge_v_cm, zorder=4, facecolor='navy')
            ...
            for j in range(4):
                ...
                txt_color = 'black' if abs(span_dist - sub_gap) < 0.1 else 'darkred'
                span_label = f"자간:\n{round(span_dist,1)}cm"
                ax3.text(mid_span_x, -H_ridge_cm*0.15, span_label, ha='center', fontsize=18, color=txt_color, weight='bold') 

                if j % 2 == 0:
                    draw_diag_poly(ax3, start_tip_r, end_tip_r, y_inner_bot_r, y_inner_top_r, w_half_r, True, zorder=3, facecolor='#D4AC0D')
                    ax3.text((start_tip_r + end_tip_r)/2, H_ridge_cm/2, f"{round(diag_len,1)}\n({int(round(cut_angle, 0))}°)", ha='center', va='center', fontsize=16, color='black', rotation=angle_rad, weight='bold') 
                else:
                    draw_diag_poly(ax3, start_tip_r, end_tip_r, y_inner_bot_r, y_inner_top_r, w_half_r, False, zorder=3, facecolor='#D4AC0D')
                    ax3.text((start_tip_r + end_tip_r)/2, H_ridge_cm/2, f"{round(diag_len,1)}\n({int(round(cut_angle, 0))}°)", ha='center', va='center', fontsize=16, color='black', rotation=-angle_rad, weight='bold') 

            ax3.annotate('', xy=(x_min, -H_ridge_cm*0.45), xytext=(x_max, -H_ridge_cm*0.45), arrowprops=dict(arrowstyle='<->', color='red', lw=2))
            ax3.text((x_min + x_max)/2, -H_ridge_cm*0.6, f"{label} 모듈 총길이: {round(L_set,1)}cm", ha='center', fontsize=22, color='darkred', weight='bold') 
    ...
    ax3.text(-gap_m*0.1, H_ridge_cm/2, f"다대 실절단\n{round(actual_ridge_v_len,1)}cm", va='center', ha='right', color='black', fontsize=24, weight='bold') 
    
    ax3.set_title(f"3. 용마루 전체 조립도 (높이: {H_ridge_cm}cm)", fontsize=36, fontweight='bold', pad=30) 
...
def main():
    ...
    app_mode = st.sidebar.radio("👉 원하시는 작업을 선택하세요:", ["1. 맞춤형 트러스 생성기", "2. 벽사다리 통합 산출 시스템"])
    
    if app_mode == "1. 맞춤형 트러스 생성기":
        st.header("1. 맞춤형 트러스(밑더블/아치 등) 생성기")
        
        col1, col2, col3 = st.columns(3)
        with col1:
            type_options = {
                "대칭 삼각 (하현부 일반)": "1", "아치형 (하현부 일반)": "2", "반삼각 (하현부 일반)": "3",
                "서브형 삼각 (하현부 경사+수평보)": "4", "서브형 아치": "5", "서브형 반삼각 (수평보 없음)": "6",
                "밑더블 삼각 (상하단 분리+W형 살대)": "7", "밑더블 아치": "8", "밑더블 반삼각": "9"
            }
            selected_type_str = st.selectbox("트러스 형태 선택", list(type_options.keys()), index=6)
            type_choice = type_options[selected_type_str]
            
            span_cm = st.number_input("전체 스판(cm)", value=1200.0)
            divs = st.number_input("등분 수(다대 개수 결정)", value=12, step=1)
        ...
        with col2:
            ...
            if is_sub_type and type_choice != "5":
                h_outer_cm = st.number_input("트러스 상하 일정 수직 폭(cm)", value=80.0)
            elif is_double_bot:
                h_outer_cm = st.number_input("밑더블 양끝 외경 시작높이(cm)", value=80.0)
            else:
                h_outer_cm = st.number_input("양쪽 끝단(시작) 높이(cm)", value=50.0)
                
            h_center_cm = st.number_input("최고점 상단 높이(cm)", value=250.0)
            
            if has_tie:
                h_tie_cm = st.number_input("수평재 바닥기준 높이(cm)", value=150.0)
            else:
                h_tie_cm = 0.0
                
        with col3:
            m_od = st.number_input("상/하현부 및 수평 가로 지름(mm)", value=59.9)
        ...
        if st.button("🚀 도면 및 재단표 생성 (트러스)", type="primary"):
            with st.spinner("생성 중입니다..."):
                fig, raw_data, t_name = generate_custom_truss(type_choice, span_cm, divs, h_outer_cm, h_center_cm, h_tie_cm, m_od, v_od, r_od, d_od, offset_mm)
                st.pyplot(fig)
                
                # 파일 저장 준비
                file_prefix = f"Truss_{t_name.replace(' ', '')}_{int(span_cm)}"
                excel_filename = f"{file_prefix}_재단표.xlsx"
                pdf_filename = f"{file_prefix}.pdf"
                
                fig.savefig(pdf_filename, bbox_inches='tight')
                save_formatted_excel(raw_data, excel_filename)
                
                c1, c2 = st.columns(2)
                with open(pdf_filename, "rb") as f:
                    c1.download_button("📄 PDF 도면 다운로드", f, file_name=pdf_filename, mime="application/pdf")
                with open(excel_filename, "rb") as f:
                    c2.download_button("📊 엑셀 재단표 다운로드", f, file_name=excel_filename, mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                
                st.success("✅ 생성 완료!")

    elif app_mode == "2. 벽사다리 통합 산출 시스템":
        st.header("2. 벽사다리 및 보강사다리 통합 산출 시스템")
        
        c1, c2, c3 = st.columns(3)
        with c1:
            st.subheader("기본 치수 (중심선 기준)")
        ...
        with c2:
            st.subheader("용마루 & 벽사다리 치수")
            H_ridge_cm = st.number_input("용마루 폭(높이)(cm)", value=70.0)
            ridge_deduct_mm = st.number_input("용마루 공제 사이즈(mm)", value=59.9)
        ...
        with c3:
            ...
            p_sub_main = st.number_input("보강 상하현재", value=38.1)
            p_sub_sub = st.number_input("보강 수직/사재", value=31.8)
            p_main_main = st.number_input("메인 상하현재", value=42.2)
            p_main_snagi = st.number_input("메인 스나기", value=89.1)
            p_main_v = st.number_input("메인 수직다대", value=38.1)
            p_main_diag = st.number_input("메인 살대", value=31.8)
            p_ridge_main = st.number_input("용마루 상하현재", value=42.2)
            p_ridge_v = st.number_input("용마루 수직다대", value=38.1)
            p_ridge_diag = st.number_input("용마루 살대", value=31.8)

        if st.button("🚀 도면 및 산출표 생성 (사다리)", type="primary"):
            with st.spinner("생성 중입니다..."):
                fig, df_raw = run_ladder_system(L_cm, W_cm, H_truss_cm, total_sets_sub, total_sets_main, offset_mm, H_ridge_cm, ridge_deduct_mm, total_sets_ridge, wall_snagi_mm, p_sub_main, p_sub_sub, p_main_main, p_main_snagi, p_main_v, p_main_diag, p_ridge_main, p_ridge_v, p_ridge_diag)
                st.pyplot(fig)
                
                pdf_filename = f"종합상세도면_{int(L_cm)}cm.pdf"
                excel_filename = f"종합산출표_{int(L_cm)}cm.xlsx"
                
                with PdfPages(pdf_filename) as pdf:
                    pdf.savefig(fig)
                    
                with pd.ExcelWriter(excel_filename, engine='openpyxl') as writer:
                    df_raw.to_excel(writer, index=False, sheet_name='전체자재표')
                ...
                with open(pdf_filename, "rb") as f:
                    c1.download_button("📄 PDF 도면 다운로드", f, file_name=pdf_filename, mime="application/pdf")
                with open(excel_filename, "rb") as f:
                    c2.download_button("📊 엑셀 자재산출표 다운로드", f, file_name=excel_filename, mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
